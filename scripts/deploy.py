...
def configure_environment(env):
    ...
    try:
        ...
        # 覆盖部署相关配置，保留数据库/API密钥等现有配置
        existing_config.update(env_config)
        
        with open(env_file, 'w') as f:
            f.write(f"# TestBrain {env} environment config\n")
            f.write(f"# Generated by deploy.py at {datetime.now().isoformat()}\n\n")
...
def main():
    ...
    success_count = 0
    failed_step = None
    deploy_backup_name = None  # 记录部署前备份，用于失败后回滚
    ...
    for func_name, step_name, enabled in steps:
        ...
        try:
            ...
            if func_name == 'pull_code':
                ...
                deploy_backup_name = func(args.env)
                success = deploy_backup_name is not None
            else:
                success = func()
    ...
    if failed_step:
        ...
        # 询问是否回滚（使用部署前创建的备份）
        if deploy_backup_name and input("\n是否执行回滚操作? (y/N): ").lower() == 'y':
            rollback_deploy(args.env, deploy_backup_name)
        elif not deploy_backup_name:
            print_status("未找到部署前备份，无法回滚", 'warning')
